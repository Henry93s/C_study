...
private:
    ...
    /*
        이 위치에서 호출할 생성자를 명시할 수 없음 => 이니셜라이저(초기화리스트) 사용
        Chulsoo, Younghee 에 디폴트 생성자가 없음. (기존 인자가 있는 생성자가 있으므로)
    */
public:
    /*
    ...
    */
    ...
    // Chulsoo, Younghee 에 추가로 기본 생성자를 작성하지 않으려면, 초기화 리스트를 사용하여 직접 맴버 객체를 초기화한다.
    /*
        초기화 리스트 : 클래스의 정의에 따른 객체 생성에서 초기화가 불가능한 
        const 나 레퍼런스 변수, 클래스 객체 인 멤버 변수를 생성자를 통해서 초기화할 때 사용
...
int main(void) {
    ...
    // or DailyLife 생성자에 이니셜라이저 를 통해 클래스 객체를 초기화
